...
`gsap.from()` straight to its end state. The page looked right and animated not at all, and
...
change the document height after first paint. That is a safety net, not the fix. Do not rely
on it to cover dynamic imports.
